...
if __name__=='__main__':
    with open('../../verifier/threshold.json', 'r') as fin:
    ...
    with open('compressedVideoNameDict.json', 'r') as dict_fin:
    ...
    for videoName, metadataName, videoType in zip(videoNameList[start:start+1], metadataList[start:start+1], videoTypeList[start:start+1]):
        with open('log_' + videoName + '.txt', 'w') as fout:
            ...
            for pp in programList:
                ...
                plan = program(pp)
                matcher = verifier(compressedVideoNameDict[videoName], metadataName, plan, threshold, sampleParam, compressParam)
    ...
    with open('result_{}.json'.format(videoNameList[start]), 'w') as result_fout:
        json.dump(confusionMatrix, result_fout)
