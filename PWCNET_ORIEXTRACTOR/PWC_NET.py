...
# arguments_strVideo = './videos/example.MP4'
# arguments_strOut_path = './extracted/'
# arguments_strStart_Frame = '0'
# arguments_strFrame_Num = '-1'
...
# 		arguments_strVideo = './videos/' + strArgument # path to the video

# 	elif strOption == '--out':
# 		arguments_strOut_path = strArgument # path to where the output should be stored

# 	elif strOption == '--start':
# 		arguments_strStart_Frame = strArgument # the frame we start to crack

# 	elif strOption == '--num':
# 		arguments_strFrame_Num = strArgument # the number of frames we crack is this thread
# 	# end
# # end
...
def generateFlo(firstFrame, secondFrame, floPath, moduleNetwork):
	tensorFirst = torch.FloatTensor(numpy.array(firstFrame)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
	tensorSecond = torch.FloatTensor(numpy.array(secondFrame)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))

	tensorOutput = estimate(tensorFirst, tensorSecond, moduleNetwork)

	objectOutput = open(floPath, 'wb')
...
##########################################################
# NOTICE that this function will compute the optical flow from #startFrame to #endFrame+1 in the video.
...
def PWC_NET(arguments_strVideo, arguments_strModel = 'sintel', arguments_strOut_path = './extracted/', arguments_strStart_Frame = '0', arguments_strFrame_Num = '-1'):
	moduleNetwork = Network(arguments_strModel).cuda().eval()
	
	cap = cv2.VideoCapture(arguments_strVideo)
	if cap.isOpened() == False:
		print('Error address:', arguments_strVideo)
		raise Exception('Cannot Open the Video.')
	# end

	startFrame = int(arguments_strStart_Frame)
	cap.set(cv2.CAP_PROP_POS_FRAMES, startFrame)

	ret1, previousFrame = cap.read()
	if ret1 == False:
		raise Exception('Cannot read the frame' + str(startFrame))
	# end

	frameNumToBeComputed = int(arguments_strFrame_Num)
	for i in range(startFrame, startFrame + frameNumToBeComputed):
		ret2, nextFrame = cap.read()
		if ret2 == False:
			raise Exception('Cannot read the frame' + str(i))
			break		
		# end

		print('---Processing frame %d and %d' % (i, i+1))
		previousFrameForPIL = PIL.Image.fromarray(cv2.cvtColor(previousFrame, cv2.COLOR_BGR2RGB))
		nextFrameForPIL = PIL.Image.fromarray(cv2.cvtColor(nextFrame, cv2.COLOR_BGR2RGB))
		# print('---Frames extracted')

		# print('---Converting .flo file')
		floName = 'flo' + str(i) + '.flo'
		generateFlo(previousFrameForPIL, nextFrameForPIL, arguments_strOut_path + floName, moduleNetwork)
		# print('---.flo converted')

		# # print('---Converting .png file')
		# pngName = 'png' + str(i) + '.png'
		# os.system('./flow-code/color_flow ' + arguments_strOut_path + floName + ' ' + arguments_strOut_path + pngName)
		# # print('---.png converted')
		print('---Done.')
