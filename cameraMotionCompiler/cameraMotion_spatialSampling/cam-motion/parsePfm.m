%Copyright (c) 2017, Pavel Tokmakov, Karteek Alahari, Cordelia Schmid
...
%   Bring a .pfm image into matlab.
    ...
    numChannels = 0; %0; %1 = grayscale, 3 = RGB
    imWidth     = 0;
    imHeight    = 0; 
    isBigEndian = 0;
    ...
    if strcmp(strtrim(line1), 'PF') == 1 %Color
        numChannels = 3;
    elseif strcmp(strtrim(line1), 'Pf') == 1 %Gray
        numChannels = 1;
    else %Invalid header
        fclose(fid);
...
%         for f = numFloatsRead + 1 : totElems
%             rawData(f) = 0;
%         end
%     end
    if numFloatsRead ~= totElems
