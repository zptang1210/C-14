function [ rotation_ABC, translation_UVW, fval ] = CameraMotion( OF, segmentation, focallength_px, R, spatialSamplingRate)
...
%         pE            pseudo projection error
    ...
    % down sample flow
    idx_motionComponent_downSampled = dSample(idx_motionComponent, spatialSamplingRate, height, width);
    ...
    %for i = 1:length(C)
        ...
        options = optimoptions('fminunc','Display','off', 'FiniteDifferenceType', 'central', ...
            'FiniteDifferenceStepSize', eps^(1/3), 'OptimalityTolerance', 1e-8);
